<div ng-controller="JsonController" ng-init="initItems('<% $field['name'] %>', '<% isset($fieldData) ? json_encode($fieldData->$field['name']) : '' %>')">
    ...
    <div class="row narrow" style="margin-bottom: 5px" ng-repeat="item in items | orderBy:<% isset($field['orderBy']) ? $field['orderBy'] : $columns[0]['name'] %>:<% isset($field['orderDir']) && $field['orderDir'] == 'desc' ? 'true' : 'false' %>">
    	<div class="col col-xs-9">
    		<div class="row narrow">
